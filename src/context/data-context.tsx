...
export const DataProvider = ({ children }: { children: ReactNode }) => {
    ...
    const addTransaction = async (transaction: Omit<Transaction, 'id'>) => {
        ...
        try {
            setTransactions(prev => [...prev, tempTransaction].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
            ...
            // 3. Success: Replace temp with real
            setTransactions(prev => prev.map(t => t.id === tempId ? newTransaction : t).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
        } catch (error) {
